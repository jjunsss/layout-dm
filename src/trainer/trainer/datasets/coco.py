...
class COCODataset(BaseDataset):
    ...
    def process(self):
        ...
        for split_coco in ["train2017", "val2017"]:
            data_list = []
            coco = COCO(
                os.path.join(self.raw_dir, f"instances_{split_coco}.json")
            )
            # for non-labels setting dataset
            if not self.labels:
                self.labels = [value['name'] for value in coco.cats.values()]
            ...
            for img_id in tqdm(sorted(coco.getImgIds())):
                ...
                # if H < W: #! maybe original paper with long height size layout. bcz generally layouts are used to application or industrial design.
                #     continue
                ...
                filtered = len(elements) != len(_elements) #! not matcing length elements (not correct data value)
                elements = _elements
